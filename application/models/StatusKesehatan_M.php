...
// extends class Model
class StatusKesehatan_M extends CI_Model
{
    ...
    {
        parent::__construct();
        $this->menu_name = 'StatusKesehatan';
        $this->table_name = 'm_StatusKesehatan';
    ...
    public function add($objx)
    {
        if (empty($objx)) {
            ...
            $this->db->trans_begin();
            $this->db->insert('detail_StatusKesehatan', $objx);
    ...
    public function get_all_data_by_id($id)
    {
        //Mendefinikan variabel
        $response = array();
        $data = $this->db->query("select * from m_iStatusKesehatan where id_kecamatan =  '$id' order by nama_StatusKesehatan ");
    ...
    public function get_all_data_by_id_akun($id)
    {
        //Mendefinikan variabel
        $response = array();
        $data = $this->db->query("select * from v_detail_StatusKesehatan where m_akun_id =  '$id' order by time_insert DESC ");
        $num_rows = $data->num_rows();
    ...
    public function get_all_data()
    {
        //Mendefinikan variabel
        $response = array();
        $data = $this->db->query("select * from status_kesehatan order by status_nama");
        $num_rows = $data->num_rows();
    ...
    public function delete_by_id($id)
    {
        //Mendefinikan variabel
        $response = array();
        $this->db->where('detail_StatusKesehatan_id', $id);
        $data = $this->db->delete('detail_StatusKesehatan');
        $str = $this->db->last_query();
        if ($this->db->trans_status() !== false) {
            $response['meta'] = array(
                "status_code" => 200,
