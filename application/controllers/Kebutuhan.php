...
class Kebutuhan extends BD_Controller
{
    ...
    public function get_all_post()
    {
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->Kebutuhan_M->count_all(),
                        "recordsFiltered" => $this->Kebutuhan_M->count_filtered(),
                        "data" => $data,
                );
    ...
    {
        $response = $this->Kebutuhan_M->get_by_id($id);
    ...
    {
        $response = $this->Kebutuhan_M->update_index($id_siswa,$index);
        $this->response($response);
    }
    public function add_post(){
        $dtusr = trim(file_get_contents('php://input'));
        $objx = json_decode(trim($dtusr), true);
        $response = $this->Kebutuhan_M->add($objx);
        $this->response($response);
      }
