...
class StatusKesehatan extends BD_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model('StatusKesehatan_M');
    ...
    {
        $list = $this->StatusKesehatan_M->get_all();
        $data = array();
        $no = $_POST['start'];
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->StatusKesehatan_M->count_all(),
                        "recordsFiltered" => $this->StatusKesehatan_M->count_filtered(),
                        "data" => $data,
                );
    ...
    {
        $response = $this->StatusKesehatan_M->get_by_id($id);
        $this->response($response);
    }
    public function get_all_data_by_id_get()
    {
        $id = $this->get('idKecamatan');
    $response = $this->StatusKesehatan_M->get_all_data_by_id($id);
        $this->response($response);
    }
    public function get_all_data_by_id_akun_get()
    {
        $id = $this->get('AkunId');
    $response = $this->StatusKesehatan_M->get_all_data_by_id_akun($id);
        $this->response($response);
    }
    public function get_all_data_get()
    {
    $response = $this->StatusKesehatan_M->get_all_data();
        $this->response($response);
    }
    public function update_index_get($id_siswa,$index)
    {
        $response = $this->StatusKesehatan_M->update_index($id_siswa,$index);
        $this->response($response);
    }
    public function add_post(){
        $dtusr = trim(file_get_contents('php://input'));
        $objx = json_decode(trim($dtusr), true);
        $response = $this->StatusKesehatan_M->add($objx);
        $this->response($response);
      }
    public function ubah_post()
    {
        $dtusr = trim(file_get_contents('php://input'));
        $objx = json_decode(trim($dtusr), true);
        $response = $this->StatusKesehatan_M->ubah($objx);
        $this->response($response);
    }
    public function delete_by_id_get(){ 
        $id = $this->get('id');
        $response = $this->StatusKesehatan_M->delete_by_id($id);
        $this->response($response);
    }
