...
class Daftar extends BD_Controller
{
    ...
    public function get_all_post()
    {
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->Daftar_M->count_all(),
                        "recordsFiltered" => $this->Daftar_M->count_filtered(),
                        "data" => $data,
                );
    ...
    public function ubah_post()
    {
        $dtusr = trim(file_get_contents('php://input'));
        $objx = json_decode(trim($dtusr), true);
        $response = $this->Daftar_M->ubah($objx);
        $this->response($response);
    }
    public function delete_by_id_get($id){ 
        $response = $this->Daftar_M->delete_by_id($id);
        $this->response($response);
    }
