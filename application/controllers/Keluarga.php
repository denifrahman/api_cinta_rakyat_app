...
class Keluarga extends BD_Controller
{
    ...
    public function get_all_post()
    {
        ...
        $output = array(
                        "draw" => $_POST['draw'],
                        "recordsTotal" => $this->Keluarga_M->count_all(),
                        "recordsFiltered" => $this->Keluarga_M->count_filtered(),
                        "data" => $data,
                );
    ...
    {
        $response = $this->Keluarga_M->get_by_id($id);
        $this->response($response);
    }
    public function get_all_data_by_id_get()
    {
        $id = $this->get('m_akun_id');
    $response = $this->Keluarga_M->get_all_data_by_id($id);
        $this->response($response);
    }
    public function update_index_get($id_siswa,$index)
    {
        $response = $this->Keluarga_M->update_index($id_siswa,$index);
        $this->response($response);
    }
    public function add_post(){
        $dtusr = trim(file_get_contents('php://input'));
        $objx = json_decode(trim($dtusr), true);
        $response = $this->Keluarga_M->add($objx);
        $this->response($response);
      }
    public function ubah_post()
    {
        $dtusr = trim(file_get_contents('php://input'));
        $objx = json_decode(trim($dtusr), true);
        $response = $this->Keluarga_M->ubah($objx);
        $this->response($response);
    }
    public function delete_by_id_get(){ 
        $id = $this->get('id');
        $response = $this->Keluarga_M->delete_by_id($id);
        $this->response($response);
    }
